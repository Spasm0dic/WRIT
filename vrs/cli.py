...
app = typer.Typer(
    ...
    # allow_extra_args + ignore_unknown_options lets subcommands take priority
    # over the callback's positional args; unmatched tokens land in ctx.args
...
def translations() -> None:
    ...
    if not trs:
        ...
        return
    conn = get_app_db()
    default = get_state(conn, "default_translation", "web")
    conn.close()
    for t in trs:
